...
import org.openmdx.dalvik.uses.sun.reflect.misc.ReflectUtil;
...
public class Introspector {
    ...
    private static final Map declaredMethodCache = Collections.synchronizedMap(new WeakHashMap());
    private static final Map<Class<?>, BeanInfo> beanInfoCache = Collections.synchronizedMap(new WeakHashMap<Class<?>, BeanInfo>());
    ...
    private static synchronized BeanInfo findExplicitBeanInfo(Class beanClass) {
        ...
        try {
            if (isSubclass(beanClass, org.openmdx.dalvik.uses.java.beans.BeanInfo.class)) {
        ...
        for (int i = 0; i < searchPath.length; i++) {
            ...
            if (!DEFAULT_INFO_PATH_NEW.equals(searchPath[i]) ||
                ...
                try {
                    ...
                    BeanInfo bi = (BeanInfo)instantiate(beanClass, fullName);
