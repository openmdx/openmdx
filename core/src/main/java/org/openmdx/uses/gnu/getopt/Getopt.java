...
package org.openmdx.uses.gnu.getopt;
...
/**
  ...
  */
public
Getopt(String progname, String[] argv, String _optstring, 
       LongOpt[] long_options, boolean long_only)
{
   String optstring = _optstring.length() == 0 ? " " : _optstring;
          ...
          if (opterr)
            {
              if (argv[optind].startsWith("--"))
                ...
                {
                  Object[] msgArgs = { progname,  
                                 Character.valueOf(argv[optind].charAt(0)).toString(), 
                                 nextchar };
                  System.err.println(MessageFormat.format(
