 ...
 *  
 *  This class has been copied into org.openmdx.uses.java.util name space and
 *  has slightly been modified.
 */
...
public class IdentityHashMap<K, V> extends AbstractMap<K, V> implements
	...
	public K findEqualKey(Object object) {
        if (object == null) {
        	return null;
        } else {
	    	for(int position = 0; position < elementData.length; position += 2) {
	            if (elementData[position] != null) {
	            	//
	            	// Look at the spot unless it is an empty one
	            	//
	            	Object key = elementData[position];
    ...
    public boolean containsKey(Object _key) {
        final Object key = _key == null ? NULL_OBJECT : _key;
        
        int index = getModuloHash(key, elementData.length);
        while (elementData[index] != key && elementData[index] != null) {
            index = nextIndex(index, elementData.length);
        }
    ...
    public boolean containsValue(Object _value) {
        final Object value = _value == null ? NULL_OBJECT : _value;
    ...
    IdentityHashMapEntry<K, V> getEntry(Object _key) {
        final Object key = _key == null ? NULL_OBJECT : _key;
     ...
     */
    @SuppressWarnings("unchecked")
    IdentityHashMapEntry<K, V> getEntry(int index) {
    ...
    public V put(K key, V value) {
        ...
        int index = getModuloHash(_key, len);

        while (elementData[index] != _key && elementData[index] != null) {
    ...
    private void rehash() {
        ...
        for (int i = 0; i < elementData.length; i = i + 2) {
            ...
            if (key != null) {
                // if not empty
                int index = getModuloHash(key, len);
                while (newData[index] != key && newData[index] != null) {
                    index = nextIndex(index, len);
                }
                newData[index] = key;
     ...
     *         this MapVouchers are only valid upon presentation of the Reka-Card.
     */
    @Override
    public V remove(Object _key) {
        ...
        index = getModuloHash(key, elementData.length);
        while (elementData[index] != key && elementData[index] != null) {
            index = nextIndex(index, elementData.length);
        }
        next = index;
