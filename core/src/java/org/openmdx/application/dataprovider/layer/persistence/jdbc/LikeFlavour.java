...
public enum LikeFlavour {
	...
	LOWER_SQL {
		...
		@Override
		protected void apply(StringBuilder clause, Collection<? super String> values, String columnName, String rawValue) {
			throw new UnsupportedOperationException("The actual database has no configuration for the requested LIKE flavour");
		}
		
	};
	
	protected abstract void apply(StringBuilder clause, Collection<? super String> values, String columnName, String rawValue);
	private static final Pattern OR = Pattern.compile("\\s*\\|\\s*");

	public static void applyAll(List<LikeFlavour> likeFlavours, StringBuilder clause, Collection<? super String> values, String columnName, String value) {
		String delimiter = "(";
